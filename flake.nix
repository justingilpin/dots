...
#    , home-manager, disko, vscode-server, nixos-hardware, ... }:
 #   let
 #     inputs = { inherit disko home-manager nixpkgs-unstable; }; # removed nixpkgs
  ...
  #        unstablePkgs = genUnstablePkgs system;
        in
          nixpkgs.lib.nixosSystem {
            inherit system;
            specialArgs = {
              inherit unstablePkgs; # removed pkgs
              # lets us use these things in modules
              customArgs = { inherit system hostname username unstablePkgs; }; # removed pkgs
            };
            modules = [
              #disko.nixosModules.disko
              #./hosts/nixos/${hostname}/disko-config.nix
