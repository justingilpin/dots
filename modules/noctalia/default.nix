...
#
# Import instead of modules/basic.
  ...
  home-manager.users.justin = { lib, ... }: {
    ...
    programs.noctalia-shell = {
      ...
      settings = {
        ...
        # screenOffTimeout is disabled because Noctalia's built-in screen-off
        # stage always turns monitors off. The custom command below keeps screen
        # off conditional on VS Code still running.
        # lockTimeout:      lock screen after 10 min
        ...
        idle = {
          ...
          customCommands     = builtins.toJSON [
            {
              timeout = 660;
              command = "pgrep -x code >/dev/null && hyprctl dispatch dpms off";
